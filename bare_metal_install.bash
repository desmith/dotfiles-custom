...
#               : https://ghaiklor.github.io/iterm-fish-fisher-osx/#automatic-installation
...
if [ $TERM_PROGRAM != "Apple_Terminal" ]; then
    red_color
    echo "Please run this script from the Terminal Application"
    exit 1
fi
...
function install_color_scheme() {
    ...
    if [[ ${answer} == "y" || ${answer} == "Y" ]]; then
        ...
        open -W ~/.dotfiles/.iterm2/current.itermcolors
...
function install_fisherman_plugins_and_themes() {
    ...
    if [[ ${answer} == "y" || ${answer} == "Y" ]]; then
        ...
        chmod +x ~/.dotfiles/fisher/plugins_installer.fish
        ~/.dotfiles/fisher/plugins_installer.fish
...
function post_install() {
    ...
    echo
    echo "1) Open iTerm -> Preferences -> Profiles -> Colors -> Presets and apply 'current' color preset"
