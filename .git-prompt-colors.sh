...
override_git_prompt_colors() {
  ...
  GIT_PROMPT_VIRTUALENV="(${DimCyan}_VIRTUALENV_${ResetColor}) "
  ...
  GIT_PROMPT_UPSTREAM=" {${DimCyan}_UPSTREAM_${ResetColor}}"

  ## _LAST_COMMAND_INDICATOR_ will be replaced by the appropriate GIT_PROMPT_COMMAND_OK OR GIT_PROMPT_COMMAND_FAIL
  PROMPT_HOST_PWD="${White}${at}${ResetColor}${Green}${Host}:${ResetColor}${Yellow}${PathShort}${ResetColor}"
  GIT_PROMPT_START_USER="\n${Magenta}${User}${ResetColor}${PROMPT_HOST_PWD}"
  GIT_PROMPT_START_ROOT="\n${Red}${User}${ResetColor}${PROMPT_HOST_PWD}"
  GIT_PROMPT_END_USER=" \n${White}${Time12h}${ResetColor} $ "
